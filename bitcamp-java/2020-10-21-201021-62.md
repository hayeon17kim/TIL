...
데이터센터는 삼성 본사와 떨어져 있다. 그렇다면 이 데이터 센터와 삼성 본사는 어떻게 연결할까? VPN을 각각 설치하여 삼성 본사만 연결 가능하게 만들고, 데이터를 주고 받을 때 특정 사람만 DBMS에 접근하도록 만들고, 직접 루트는 접근 못하게 막으면 된다. 직접 연결을 하는 것이 아니라 망 자체가 인터넷 망 안에 있기 때문에 물리적으로 접근하는 것을 막을 방법은 없다. 다만, 소프트웨어적으로 보안을 한 번 더 적용할 뿐이다. 회사의 네트워크 망을 직접 구축하지 않는 이상 완벽한 보안은 불가능하다. 
...
내가 있는 곳은 리눅스 서버에 접속한 상태이다. 내 컴퓨터가 아니다. 그쪽  서버에서 mysql을 실행하고 있다. 루트는 로컬로 접속한다.

내 컴퓨터에서 다이렉트로 그쪽 서버에 접근하려면 안된다. 루트가 아니라 로컬 호스트만 가능핟. 접속하려면 telnet으로 보안을 한다면  로컬에서... 루트 암호를 빼갖고 가면 다른 니 데이터 작살난다. 리눅스 서버에 설치할 때는 DBMS루트로 접근 못하게 막는 게 기본이다.

루트가 원격으로 접근하게 허락할 것인지, disallowed할 것인지. 원격에 접속하는 것은 막아야 한다. 대신 일반 사용자(이 데이터 베이스만 쓰고) 사용자를 새로 만들고 특정데이터베이스만 쓸 수 있겠금 권한을 줄여놓으면 접근할 수 있다. 
...
root로 접속하지 않고 새 데이터 베이스(엑셀 새 파일과 같다)를 만든다.


- root는 관리자 아이디이다. 

- 관리자 아이디로는 mysql의 모든 명령어를 사용할 수 있다.

- local은 같은 컴퓨터를 말한다.

- 우리는 DBMS 로컬 ServerApp으로 원격접속하고, 그럼 로컬 상태가 된다. 로컬 상태가 되기 때문에 mysql의 명령어를 사용해서 데이터에 접근할 수 있다.


![image](https://user-images.githubusercontent.com/50407047/96673627-df2f1800-13a1-11eb-92d7-d06d491f6913.png)

보통 프로그램을 실행을 하면 실행을 할 때까지 프롬프트가 안 떨어진다. 종료되어야지 떨어지지. 그러나 

os 시작할 때 실행하고 os종료할 때 종료되는 프로그램을 back-end프로그램이라고 한다. 이 backend프로그램은 

- 화면에 출력하는 것이 아니라 log 파일에 출력
- `service mariadb start`:별도 프로세스로 백엔드에서 실행하도록 만든다. 이걸 하면 눈에는 안보이지만 mariaDB 백엔드에서 실행하고 있다. ServerApp처럼 눈에는 안보이지만. `mariadb stop` 백엔드 실행되던 게 종료된다. 
- 클라이언트는 명령ㅇ어를 단순히 보낸다. 하는 게 없다. 사용자에게 입력받은 걸 서버에 보내고 서버가 결과를 보내면 출력할 뿐이다. 우리 클라이언트 프로그램처럼
- `mysql -uroot -p`: 클라이언트 프로그램이다. 
  - `mysqld`: 여기서 d는 daemon의 약자이다.
- 접속했다. Welcome to 어쩌구는 서버가 보내주는 것이다.
- `%`: 원격에서 접속
- `localhost`: 
...
- 
- 만약 회원이 탈퇴한다면? 탈퇴회원의 모든 데이터를 지우는 것이 아니다. 이름에 '탈퇴회원'이렇게 넣고 안 쓴다는 의미로 탈퇴상태에  0, F, N 등 불린 타입으로. 데이터를 지우지 않는다. 다른 사람과 엮여 있는 데이터가 많으니까 정보만 다른 값으로 덮어쓴다. 댓글은 단 사람의 소유이기 때문에 게시글에 관련되어서는 각각 댓글ㅇ르 단 사람의 것이지 글 쓴 사람의 것이 아니기 때문이다.
